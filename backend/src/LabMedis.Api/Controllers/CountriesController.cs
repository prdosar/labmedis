...
public class CountriesController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<PagedResult<CountryDto>>> GetAll([FromQuery] int page = 1, [FromQuery] int size = 10, CancellationToken ct = default)
        => Ok(await _service.GetAllAsync(page, size, ct));

    [HttpGet("select")]
    public async Task<ActionResult<IReadOnlyList<CountryDto>>> GetForSelect(CancellationToken ct)
    ...
    [HttpPost]
    public async Task<ActionResult<CountryDto>> Create([FromBody] CountryCreateDto dto, CancellationToken ct)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<ActionResult<CountryDto>> Update(long id, [FromBody] CountryUpdateDto dto, CancellationToken ct)
    {
