...
public class CategoriesController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<PagedResult<CategoryDto>>> GetAll([FromQuery] int page = 1, [FromQuery] int size = 10, CancellationToken ct = default)
        => Ok(await _service.GetAllAsync(page, size, ct));

    [HttpGet("select")]
    public async Task<ActionResult<IReadOnlyList<CategoryDto>>> GetForSelect(CancellationToken ct)
    ...
    [HttpPut("{id:long}")]
    public async Task<ActionResult<CategoryDto>> Update(long id, [FromBody] CategoryUpdateDto dto, CancellationToken ct)
    {
