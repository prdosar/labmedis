import { useState, useEffect, useMemo } from 'react'
import { FileText, ChevronsUpDown, TrendingDown, TrendingUp, RotateCcw } from 'lucide-react'
import type { CustomerDto, InvoiceDto, CustomerCreditNoteDto } from '../../api/types'
import { customersApi, invoicesApi, customerCreditNotesApi } from '../../api/endpoints'

const inputClass =
  'rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20'
const selectClass = `${inputClass} pr-8 appearance-none cursor-pointer`

function fmtXof(n: number) {
  return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'XOF', maximumFractionDigits: 0 }).format(n)
}

function fmtDate(s: string) {
  try { return new Date(s).toLocaleDateString('fr-FR') } catch { return s }
}

interface AccountLine {
  date: string
  type: 'invoice' | 'payment' | 'creditNote'
  reference: string
  label: string
  amount: number
  balance: number
}

function buildStatement(invoices: InvoiceDto[], creditNotes: CustomerCreditNoteDto[]): AccountLine[] {
  const ops: Omit<AccountLine, 'balance'>[] = []

  for (const inv of invoices) {
    // Une facture Cancelled ne débite pas le client
    if (inv.status === 'Cancelled' || inv.status === 'Draft') continue
    ops.push({
      date: String(inv.invoiceDate).slice(0, 10),
      type: 'invoice',
      reference: inv.reference,
      label: `Facture ${inv.reference}`,
      amount: inv.totalTtc,
    })
    for (const p of inv.payments) {
      ops.push({
        date: String(p.paymentDate).slice(0, 10),
        type: 'payment',
        reference: p.reference ?? inv.reference,
        label: `Règlement${p.paymentMethod ? ` (${p.paymentMethod})` : ''} / ${inv.reference}`,
        amount: -p.amount,
      })
    }
  }

  for (const cn of creditNotes) {
    if (cn.status === 'Annulé') continue
    ops.push({
      date: String(cn.creditNoteDate).slice(0, 10),
      type: 'creditNote',
      reference: cn.reference,
      label: `Avoir ${cn.reference}${cn.invoiceReference ? ` / ${cn.invoiceReference}` : ''}`,
      amount: -cn.totalAmountTtc,
    })
  }

  ops.sort((a, b) => a.date.localeCompare(b.date))

  let running = 0
  return ops.map(op => {
    running += op.amount
    return { ...op, balance: running }
  })
}

export function CustomerAccountPage() {
  const [customers, setCustomers] = useState<CustomerDto[]>([])
  const [customerId, setCustomerId] = useState<number | ''>('')
  const [invoices, setInvoices] = useState<InvoiceDto[]>([])
  const [creditNotes, setCreditNotes] = useState<CustomerCreditNoteDto[]>([])
  const [loading, setLoading] = useState(false)
  const [loaded, setLoaded] = useState(false)

  useEffect(() => {
    customersApi.getForSelect().then(setCustomers).catch(() => {})
  }, [])

  useEffect(() => {
    if (!customerId) { setInvoices([]); setCreditNotes([]); setLoaded(false); return }
    setLoading(true)
    Promise.all([
      invoicesApi.getAll(1, 500, customerId as number),
      customerCreditNotesApi.getAll({ customerId: customerId as number, size: 500 }),
    ])
      .then(([invRes, cnRes]) => {
        setInvoices(invRes.items)
        setCreditNotes(cnRes.items)
        setLoaded(true)
      })
      .catch(() => {})
      .finally(() => setLoading(false))
  }, [customerId])

  const lines = useMemo(() => buildStatement(invoices, creditNotes), [invoices, creditNotes])

  const activeInvoices = invoices.filter(i => i.status !== 'Cancelled' && i.status !== 'Draft')
  const activeCreditNotes = creditNotes.filter(c => c.status !== 'Annulé')
  const totalInvoiced = activeInvoices.reduce((s, i) => s + i.totalTtc, 0)
  const totalPaid = activeInvoices.reduce((s, i) => s + i.amountPaid, 0)
  const totalCredited = activeCreditNotes.reduce((s, c) => s + c.totalAmountTtc, 0)
  const totalBalance = totalInvoiced - totalPaid - totalCredited

  const customer = customers.find(c => c.id === customerId)

  return (
    <div className="flex flex-col gap-5">
      {/* Selector */}
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative min-w-64">
          <select
            value={customerId}
            onChange={e => setCustomerId(e.target.value ? Number(e.target.value) : '')}
            className={selectClass + ' w-full'}
          >
            <option value="">— Sélectionner un client —</option>
            {customers.map(c => (
              <option key={c.id} value={c.id}>{c.name}</option>
            ))}
          </select>
          <ChevronsUpDown size={13} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
        </div>
      </div>

      {!customerId && !loading && (
        <div className="flex flex-col items-center justify-center py-20 text-gray-400">
          <FileText size={40} className="mb-3 opacity-30" />
          <p className="text-sm">Sélectionnez un client pour afficher son relevé de compte</p>
        </div>
      )}

      {loading && (
        <div className="flex justify-center py-16">
          <span className="w-8 h-8 border-2 border-brand-500 border-t-transparent rounded-full animate-spin" />
        </div>
      )}

      {loaded && !loading && (
        <div className="flex flex-col gap-4">
          {/* Summary */}
          <div className="grid grid-cols-4 gap-4">
            <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
              <p className="text-xs text-gray-500 font-medium mb-1">Total facturé</p>
              <p className="text-lg font-bold text-gray-900">{fmtXof(totalInvoiced)}</p>
              <p className="text-xs text-gray-400 mt-0.5">{activeInvoices.length} facture{activeInvoices.length !== 1 ? 's' : ''}</p>
            </div>
            <div className="bg-white rounded-xl border border-green-100 p-4 text-center">
              <p className="text-xs text-green-600 font-medium mb-1">Total encaissé</p>
              <p className="text-lg font-bold text-green-700">{fmtXof(totalPaid)}</p>
            </div>
            <div className="bg-white rounded-xl border border-amber-100 p-4 text-center">
              <p className="text-xs text-amber-600 font-medium mb-1">Total avoirs</p>
              <p className="text-lg font-bold text-amber-700">{fmtXof(totalCredited)}</p>
              <p className="text-xs text-gray-400 mt-0.5">{activeCreditNotes.length} avoir{activeCreditNotes.length !== 1 ? 's' : ''}</p>
            </div>
            <div className={`rounded-xl border p-4 text-center ${totalBalance > 0 ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-200'}`}>
              <p className={`text-xs font-medium mb-1 ${totalBalance > 0 ? 'text-red-600' : 'text-gray-500'}`}>Solde restant dû</p>
              <p className={`text-lg font-bold ${totalBalance > 0 ? 'text-red-700' : 'text-gray-400'}`}>{fmtXof(totalBalance)}</p>
            </div>
          </div>

          {/* Statement table */}
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-2">
              <FileText size={15} className="text-gray-400" />
              <span className="text-sm font-semibold text-gray-700">
                Relevé de compte — {customer?.name}
              </span>
              <span className="text-xs text-gray-400 ml-1">{lines.length} opération{lines.length !== 1 ? 's' : ''}</span>
            </div>

            {lines.length === 0 ? (
              <p className="px-5 py-10 text-sm text-gray-400 text-center">Aucune opération enregistrée.</p>
            ) : (
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
                  <tr>
                    <th className="px-4 py-2.5 text-left w-28">Date</th>
                    <th className="px-4 py-2.5 text-left">Libellé</th>
                    <th className="px-4 py-2.5 text-right w-40">Débit</th>
                    <th className="px-4 py-2.5 text-right w-40">Crédit</th>
                    <th className="px-4 py-2.5 text-right w-40">Solde</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {lines.map((line, i) => {
                    const isDebit = line.type === 'invoice'
                    return (
                      <tr key={i} className="hover:bg-gray-50/50">
                        <td className="px-4 py-2.5 text-gray-500 text-xs">{fmtDate(line.date)}</td>
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-2">
                            {line.type === 'invoice' && <TrendingUp size={13} className="text-red-400 shrink-0" />}
                            {line.type === 'payment' && <TrendingDown size={13} className="text-green-500 shrink-0" />}
                            {line.type === 'creditNote' && <RotateCcw size={13} className="text-amber-500 shrink-0" />}
                            <span className="text-gray-800">{line.label}</span>
                          </div>
                        </td>
                        <td className="px-4 py-2.5 text-right">
                          {isDebit
                            ? <span className="font-semibold text-gray-900">{fmtXof(line.amount)}</span>
                            : <span className="text-gray-300">—</span>
                          }
                        </td>
                        <td className="px-4 py-2.5 text-right">
                          {!isDebit
                            ? <span className={`font-semibold ${line.type === 'creditNote' ? 'text-amber-700' : 'text-green-600'}`}>{fmtXof(-line.amount)}</span>
                            : <span className="text-gray-300">—</span>
                          }
                        </td>
                        <td className="px-4 py-2.5 text-right">
                          <span className={`font-semibold ${line.balance > 0 ? 'text-red-600' : line.balance < 0 ? 'text-green-600' : 'text-gray-400'}`}>
                            {fmtXof(line.balance)}
                          </span>
                        </td>
                      </tr>
                    )
                  })}
                </tbody>
                <tfoot className="border-t-2 border-gray-200 bg-gray-50">
                  <tr>
                    <td colSpan={2} className="px-4 py-3 text-sm font-semibold text-gray-700">Totaux</td>
                    <td className="px-4 py-3 text-right font-bold text-gray-900">{fmtXof(totalInvoiced)}</td>
                    <td className="px-4 py-3 text-right font-bold text-green-700">{fmtXof(totalPaid + totalCredited)}</td>
                    <td className={`px-4 py-3 text-right font-bold ${totalBalance > 0 ? 'text-red-600' : totalBalance < 0 ? 'text-green-600' : 'text-gray-400'}`}>
                      {fmtXof(totalBalance)}
                    </td>
                  </tr>
                </tfoot>
              </table>
            )}
          </div>
        </div>
      )}
    </div>
  )
}
